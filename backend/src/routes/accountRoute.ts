...
// Create account
accountRouter.post("/create", authMiddleware, async (req: AuthRequest, res: Response) => {
...
//Delete account
accountRouter.delete("/delete/:id", authMiddleware, async (req: AuthRequest, res: Response) => {
...
accountRouter.get("/all", authMiddleware, async (req: AuthRequest, res: Response) => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
...
accountRouter.get("/accounts/:id", authMiddleware, async (req: AuthRequest, res: Response) => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
...
accountRouter.get("/accounts/balance/:id", authMiddleware, async (req: AuthRequest, res: Response) => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
